...
// data.ts or inside component file
export const columns: Column[] = [
  { title: "Product", items: ["Changelog", "Engineering", "Docs", "Bounce doctor"] },
  { title: "Resources", items: ["Guides", "Customers", "Glossary", "Email examples"] },
  { title: "Company", items: ["About Us", "Updates", "Careers", "Credits"] },
...
export default function Footer() {
    return (
        <footer className="max-w-screen flex justify-center items-center border-t border-t-neutral-300 border-b-2 border-b-orange-500 mt-10">
             ...
             <div className=" w-full flex justify-evenly">
                {
                    columns.map((column)=> {
                          return(
                              <div key={column.title} className="flex flex-col justify-center gap-2.5">
                                   <p key={column.title} className="text-neutral-600 text-sm font-bold  cursor-pointer">{column.title}</p>
                                   {column.items.map((item)=>{
                                    return(
                                        <p className="text-neutral-400 text-[15px] font-medium hover:font-semibold cursor-pointer">{item}</p>
                                    )
                                   })}
                              </div>
                          )
            ...
            <div className="bg-neutral-300 w-full h-[1px]"/>
            
            <div className="flex justify-between mt-8 mb-16">
                  <p className="text-sm text-neutral-400 tracking-wide">© 2025 Astrodon Inc.</p>
                  <div className="flex justify-evenly w-[50%]">
                        <p className="text-sm text-neutral-400 tracking-wide"> Privacy Policy</p>

                        <p className="text-sm text-neutral-400 tracking-wide">Terms of Service</p>

                        <p className="text-sm text-neutral-400 tracking-wide">Trust Center</p>

                        <p className="text-sm text-neutral-400 tracking-wide">DPA</p>

                        <p className="text-sm text-neutral-400 tracking-wide">DPF</p>
                  </div>
            </div> 
            </div>
